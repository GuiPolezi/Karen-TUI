...
class HealthScreen(ModeScreen):
    ...
    def refresh_health(self) -> None:
        ...
        extra.append("Dados  ", style=label).append(info.get("data_dir", "?")).append("\n")
        update = info.get("update", "")
        extra.append("Atualização  ", style=label).append(
            update,
            style=tokens.rich("accent") if ("novos" in update or "disponível" in update) else tokens.rich("text"),
        ).append("\n")
